...
async function main() {
  ...
  try {
    ...
    const streamRoute = await import("../src/app/api/v1/jobs/[id]/stream/route").catch(() => null);
    assert.ok(statusRoute && cancelRoute && streamRoute, "Job status, cancel and stream routes must exist");
    ...
    const streamResponse = await streamRoute.GET(
      new Request("https://sushua.test/api/v1/jobs/0199aa99-1111-7111-8111-111111111111/stream"),
      context,
    );
    ...
    assert.equal((await streamResponse.json()).error.code, "not_found");
