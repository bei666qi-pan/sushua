...
| `async_ingestion` | `POST /api/v1/uploads` 创建 Document 草稿并返回 S3 multipart 直传计划；`POST /api/v1/uploads/{asset_id}/complete` 校验对象并原子创建 `file.scan` Job；仅在迁移 `0013`、RLS Web 角色、函数授权和 S3 配置就绪后开启 | 两个 API 都在初始化 Auth、PostgreSQL 或 Storage 前返回 404；旧 `/api/parse` 不变 | 已有 Document、DocumentVersion、SourceAsset、multipart 状态与 Job 保留 |
