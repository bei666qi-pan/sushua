...
| `async_ingestion` | `POST /api/v1/uploads` 创建 Document 草稿并返回 S3 multipart 直传计划；仅在迁移 `0012`、RLS Web 角色和 S3 配置就绪后开启 | API 在初始化 Auth、PostgreSQL 或 Storage 前返回 404；旧 `/api/parse` 不变 | 已有 Document、DocumentVersion、SourceAsset 与 multipart 状态保留 |
