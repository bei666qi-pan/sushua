...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  const authEnabled = isFeatureEnabled("guest_claim");
  const workspaceEnabled = isFeatureEnabled("workspace_library");
  return (
    <html lang="zh-CN">
          ...
          <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-4 sm:px-6">
            ...
            <nav className="flex items-center gap-1.5 sm:gap-3">
              ...
              {workspaceEnabled && (
                <Link
                  href="/workspaces"
                  className="rounded-lg px-2.5 py-1.5 text-sm text-ink-soft transition-colors hover:bg-pine-soft hover:text-pine"
                >
                  资料库
                </Link>
              )}
