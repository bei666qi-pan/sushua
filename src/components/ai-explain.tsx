    ...
    <div className="fade-up mt-3 overflow-hidden rounded-xl border border-pine/20 bg-card">
      ...
      <div className="flex items-center justify-between border-b border-line bg-pine-soft/50 px-3.5 py-2">
        <span className="flex items-center gap-1.5 text-xs font-medium text-pine">
          ...
          {cached && version < CURRENT_VERSION && (
            <span className="rounded-full bg-warn-soft px-1.5 py-0.5 text-[10px] text-ink-soft">旧版·可重新生成</span>
          )}
          {streaming && !text && <span className="text-ink-faint">思考中…</span>}
        </span>
        {(status === "done" || status === "error") && (
          <button onClick={() => run(true)} className="text-xs text-ink-faint transition-colors hover:text-pine">
            重新生成
          </button>
        )}
      </div>
      ...
      {status === "error" ? (
        ...
        <div className="space-y-2.5 px-3.5 py-3">
          ...
          {sections.doubt && (
            <div className="rounded-lg bg-warn-soft px-3 py-2 text-[13px] leading-relaxed text-ink-soft">
              ⚠ 答案存疑:{sections.doubt}
            </div>
          )}
                    ...
                    {m ? (
                      <>
                        ...
                        <span className={streaming && i === sections.traps.length - 1 ? "stream-cursor" : ""}>
                          {mine && (
                            <span className="mr-1.5 rounded bg-bad-soft px-1 py-0.5 text-[10px] font-medium text-bad">你选的</span>
                          )}
                          {m[2]}
                        </span>
                      </>
          ...
          {/* 速记口诀 */}
          {sections.memo && (
            <div className="flex items-start gap-2 rounded-lg bg-pine-soft px-3 py-2">
              <span className="mt-0.5 shrink-0 text-xs">📌</span>
              <p className={`text-[13px] font-medium leading-relaxed text-pine ${streaming ? "stream-cursor" : ""}`}>
                {sections.memo}
              </p>
            </div>
          )}
